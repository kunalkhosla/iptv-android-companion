...
// Compose stability story:
// These models are deserialized once per network response and never
// mutated in place — `data class` with `val` fields. We annotate the
// frequently-rendered ones @Immutable so Compose's smart-skipping can
// avoid recomposition when reference equality holds. `List<X>` fields
// would otherwise mark the whole class @Unstable (List is mutable in
// Kotlin's eyes) and force re-render on every parent recomposition.
//
// `@Immutable` is a contract — these classes promise their values
// (and the values inside any List) never change after construction.
// We do honor it: the home payload and rail items are replaced via
// new instances rather than mutated.
...
@Serializable
data class HealthResponse(val ok: Boolean = true, val sha: String? = null)
...
data class HomeItem(
    ...
    val container: String? = null,
    // Source audio channel count (2 / 6 / 8) when the server has
    // probed it. Drives the "5.1" / "7.1" badge on the tile so the
    // user can spot titles that'll exercise their home-theater setup.
...
// Server expects `{position, duration}` (matches web client at
// public/app.js postProgress). Storage shape on disk uses `{p, d, t}`,
// but the request body is the verbose form. Sending `{p, d}` here
// returned a 400 silently and every series progress post was lost —
// movies happened to be saved via the web client so the bug masked
// itself until series episodes started failing to resume.
@Serializable
