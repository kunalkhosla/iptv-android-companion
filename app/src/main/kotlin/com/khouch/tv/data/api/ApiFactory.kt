...
class ApiFactory(
    ...
    // 50 MB HTTP cache. The server sets Cache-Control: private, max-age=N
    // on the cacheable detail endpoints (poster / similar / series info /
    // category), so OkHttp transparently serves repeats from disk without
    // a network round-trip. Combined with the repository-level TTL caches,
    // this gives us three layers: in-memory (process lifetime, TTL'd),
    // OkHttp disk (across restarts, respects Cache-Control), and Coil
    // disk (image bytes).
    // Public so the Settings screen can display size + evict on demand.
    val httpCache: Cache by lazy {
        Cache(File(context.cacheDir, "okhttp"), 50L * 1024 * 1024)
    }
